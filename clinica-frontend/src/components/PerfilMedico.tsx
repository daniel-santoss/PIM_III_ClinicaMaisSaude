...
export default function PerfilMedico() {
  ...
  const [especialidades, setEspecialidades] = useState<string[]>(["Clínico Geral", "Cardiologia"]);
  ...
  useEffect(() => {
    // Simulando busca de dados do médico
    setTimeout(() => {
      setMedico({
        nome: "Dr. Ricardo Oliveira",
        crm: "123456-SP",
        email: "ricardo.med@clinicamaissaude.com"
      });
      setCarregando(false);
    }, 800);
  }, []);
  ...
  const especialidadesFiltradas = ESPECIALIDADES.filter(
    esp => esp.toLowerCase().includes(busca.toLowerCase()) && !especialidades.includes(esp)
  );
    ...
    <div className="animate-in fade-in slide-in-from-top-4 duration-500 max-w-2xl mx-auto space-y-10">
      ...
      <div className="flex items-center gap-6">
        ...
        <div>
          ...
          <p className="text-gray-400 text-xs font-bold uppercase tracking-widest">CRM: {medico?.crm}</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between ml-2">
          ...
          <span className="text-[10px] font-bold text-purple-500 bg-purple-50 px-2 py-1 rounded-lg">{especialidades.length} selecionadas</span>
        </div>
        ...
        <div className="bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            {especialidades.map(esp => (
              <span key={esp} className="inline-flex items-center gap-2 bg-purple-50 text-[#7C3AED] px-4 py-2 rounded-xl text-xs font-black border border-purple-100 group animate-in zoom-in duration-200">
                {esp}
                <button onClick={() => removerEspecialidade(esp)} className="hover:text-red-500 transition-colors">
      ...
      {/* Ações Finais */}
      <div className="pt-6">
        <button className="w-full py-4 bg-gray-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all shadow-lg active:scale-95">
          Salvar Alterações
        </button>
      </div>
    </div>
