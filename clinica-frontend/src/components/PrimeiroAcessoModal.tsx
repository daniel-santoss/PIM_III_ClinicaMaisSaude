...
/**
 * Primeiro acesso do auto-cadastro moderado (web): o proponente APROVADO conclui o cadastro.
 * Pede o código (por CPF ou e-mail), confirma com o código + CPF e define a senha — aí a conta é
 * criada e ele já pode entrar. Gêmeo funcional do RecuperarSenhaModal (mesma máquina de código).
 */
export default function PrimeiroAcessoModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const fechar = () => {
    ...
    setIdentificador(""); setIsCpfMask(false); setCodigo(""); setCpf(""); setResetToken("");
  ...
  const solicitar = async (e: React.FormEvent) => {
    ...
    // Se o identificador é um CPF, já prepara a confirmação do passo 2.
    if (!/[a-zA-Z@]/.test(identificador)) setCpf(mascaraCpf(identificador));
    try {
      await fetch(`${API_URL}/api/AutoCadastro/primeiro-acesso/solicitar`, {
        method: "POST",
  ...
  const confirmarCodigo = async (e: React.FormEvent) => {
    ...
    if (soDigitos(cpf).length !== 11) { setErro("Confirme seu CPF (11 dígitos)."); return; }
    setErro(""); setCarregando(true);
    try {
      const res = await fetch(`${API_URL}/api/AutoCadastro/primeiro-acesso/confirmar`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ cpf: soDigitos(cpf), codigo }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(data?.message || data?.detail || "Código inválido ou expirado.");
      }
  ...
  const definirSenha = async (e: React.FormEvent) => {
    ...
    setErro(""); setCarregando(true);
    try {
      const res = await fetch(`${API_URL}/api/AutoCadastro/primeiro-acesso/definir-senha`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => null);
        throw new Error(data?.message || data?.detail || "Não foi possível criar a conta.");
      }
      setPasso("sucesso");
    } catch (err: any) {
  ...
  const tituloPorPasso = {
    identificador: "Primeiro acesso",
    codigo: "Confirme o código",
  ...
  return createPortal(
    <div className="fixed inset-0 z-[3000] flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={fechar}>
      ...
      >
        ...
        {passo === "identificador" && (
          <form onSubmit={solicitar} className="mt-5 space-y-4 text-left">
            <p className="text-sm font-medium text-gray-500 text-center">
              Seu cadastro foi aprovado? Informe seu CPF ou e-mail para receber o código de primeiro acesso.
            </p>
            <input
              type="text"
              autoFocus
        ...
        {/* Passo 2 — código + CPF */}
        {passo === "codigo" && (
          <form onSubmit={confirmarCodigo} className="mt-5 space-y-4 text-left">
            <p className="text-sm font-medium text-gray-500 text-center">
              Se o cadastro estiver aprovado, enviamos um código de 6 caracteres ao e-mail. Digite-o e confirme seu CPF.
            </p>
            <input
              type="text"
              inputMode="text"
        ...
        {passo === "senha" && (
          <form onSubmit={definirSenha} className="mt-5 space-y-4 text-left">
            <p className="text-sm font-medium text-gray-500 text-center">
              Defina a senha de acesso da sua conta (mínimo {SENHA_MIN} caracteres).
            </p>
        ...
        {passo === "sucesso" && (
          <div className="mt-4">
            <p className="text-sm font-medium text-gray-600 leading-relaxed mb-6">
              Sua conta foi criada com sucesso. Você já pode entrar com seu CPF/e-mail e a senha que definiu.
            </p>
