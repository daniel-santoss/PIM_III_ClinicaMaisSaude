...
- ⏭️ **Fases 6–8 pendentes.** Próxima: **Fase 6 (Penalidade de IA)** — `BloqueadoIAAte` migra `Paciente`→`LoginPortal`; `PenalidadeRemovidaAvisar` deixa de existir → vira `Notificacao` (some `ConsumarAvisoPenalidade`/`LoginResponse.PenalidadeRemovida`).
