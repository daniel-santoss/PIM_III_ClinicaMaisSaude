...
- ⏭️ **Fases 5–8 pendentes.** Próxima: **Fase 5 (FKs faltantes em `Agendamento`)** — `ProfissionalId`→Profissional, `AgendamentoOrigemId`→self, `EspecialidadeId`→Especialidade (mudar `int?`→`EspecialidadeMedica?`).
